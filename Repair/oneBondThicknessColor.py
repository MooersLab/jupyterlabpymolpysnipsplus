"""
cmd.do('# To change stick color and radius for the bond between atom 2 and 3;')
cmd.do('set_bond stick_color, ${1:yellow}, index 2, index 3;')
cmd.do('set_bond stick_radius, ${2:0.2}, index 2, index 3;')
"""
cmd.do('# To change stick color and radius for the bond between atom 2 and 3;')
cmd.do('set_bond stick_color, yellow, index 2, index 3;')
cmd.do('set_bond stick_radius, 0.2, index 2, index 3;')

# Description:  To change stick color and radius for the bond between atom 2 and 3, use the set_bond command.

# Source:  https://www.mail-archive.com/pymol-users@lists.sourceforge.net/msg15584.html

